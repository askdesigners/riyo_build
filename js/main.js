...
UI.prototype = {
  ...
  init: function() {
    ...
    this.$slideshowContainer = $('#home-slideshow');
    this.$slideshowTitle = $('#home-slideshow-title');
  ...
  bindEvents: function(isMobile) {
    ...
    this.$slideshowContainer.on('cycle-after', function(e, opts) {
      ...
      if (!self.$slideshowContainer.is('.cycle-paused')) progress.animate({ width: '100%' }, opts.timeout, 'linear');
    });
  },
  ...
  showContentModal: function(event, clickedItem) {
    ...
    self.$modalContainer.find('.modalInner').load(clickedItem.href, function() {
      ...
      self.$modalContainer.find('.close').on('click', function() {
        self.$modalContainer.fadeOut('fast', function() {
          ...
          self.$modalContainer.find('.modalInner').empty();
        });
      });
    });
  },
  ...
  updateSlideshowTitle: function(event, optionHash, slideOptionsHash, currentSlideEl) {
    ...
    this.$morebutton.val(slideOptionsHash.title);
  }
};
